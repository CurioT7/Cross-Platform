...
Future<double?> showSortPostsBottomSheet(
    BuildContext context,
    String initialSort,
    IconData selectedIcon,
    Function(String, IconData) updateSortAndIcon,
    Function(String) fetchPosts,
  ...
  showModalBottomSheet(
    ...
    builder: (BuildContext context) {
      ...
      IconData selectedSortIcon =
          selectedIcon; // Store the selected icon separately
            ...
            children: [
              AppBar(
                ...
                actions: [
                  TextButton(
                    onPressed: () {
                      updateSortAndIcon(selectedSort,
                          selectedSortIcon); // Pass the selected icon
                      print('Selected Sort: $selectedSort');
                      fetchPosts(
                          selectedSort); // Fetch posts based on the selected sorting criteria
...
//print timeSelected
                          timeSelected
                              .then((value) => print('Time Selected: $value'));
                          print('Selected Sort: $selectedSort');
                          selectedIcon = Icons
                              .arrow_upward_sharp; // Keep the same icon for 'Top'.
                        });
                      });
                    } else {
                      ...
                      setModalState(() {
                        selectedSort =
                            value ?? ''; // Ensure the value is non-nullable.
                        selectedIcon = item[
                            'icon']; // Update the icon for the selected sort type.
                      });
                    }
                  },
                );
